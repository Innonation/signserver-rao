...
from api.utils import check_authtoken, check_pin, check_hmac
        ...
        def onCall(request, *args, **kwargs):
            ...
            try:
                ...
                if not user:
                    LOG.error('Utente non trovato Username: {}'.format(str(username.upper())))
                    return JsonResponse({'statusCode': StatusCode.UNAUTHORIZED.value, 'message': 'Accesso negato'})
                if check_authtoken(username, entity, new_username, str(authToken)):
        ...
        def onCall(request, *args, **kwargs):
            ...
            try:
                ...
                if not user:
                    LOG.error('Utente non trovato Username: {}'.format(str(username.upper())))
                    return JsonResponse({'statusCode': StatusCode.UNAUTHORIZED.value, 'message': 'Accesso negato'})
                if check_authtoken(username, entity, newPin, str(authToken)):
                    return function(request, *args, **kwargs)
            except Exception as e:
                LOG.error(
                    'Authorization Header non presente o non valido. AuthToken: {} - Errore: {}'.format(str(authToken),
    ...
    def decorator(function):
        def onCall(request, *args, **kwargs):
            ...
            try:
                ...
                if not (api_data or authToken) or not username or not payload:
                    LOG.error("Parametri assenti")
                    return JsonResponse({'statusCode': StatusCode.UNAUTHORIZED.value, 'message': 'Accesso negato'})
                entity_obj = Entity.objects.filter(entity=entity.upper()).last()
                if not entity_obj:
                    LOG.error('Codice IPA non trovato: {}'.format(str(entity.upper())))
                    return JsonResponse({'statusCode': StatusCode.UNAUTHORIZED.value, 'message': 'Accesso negato'})
                user = User.objects.filter(username=username.upper(), status=True, entity=entity_obj, role=RoleTag.OPERATOR.value).first()
                if not user:
                    LOG.error('Utente non trovato Username: {}'.format(str(username.upper())))
                    return JsonResponse({'statusCode': StatusCode.UNAUTHORIZED.value, 'message': 'Accesso negato'})

                if (api_data and check_hmac(username, api_data)) or check_authtoken(username, entity, payload, str(authToken)):
                    user.failureCounter = 0
                    user.save()
                    return function(request, *args, **kwargs)
                else:
        ...
        def onCall(request, *args, **kwargs):
            ...
            try:
                ...
                if not user:
                    LOG.error('Utente non trovato Username: {}'.format(str(username.upper())))
                    return JsonResponse({'statusCode': StatusCode.UNAUTHORIZED.value, 'message': 'Accesso negato'})
                if check_authtoken(username, entity, cert, str(authToken)):
                    user.failureCounter = 0
                    user.save()
                    return function(request, *args, **kwargs)
                else:
